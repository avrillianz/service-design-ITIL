    ...
    <div class="container-fluid">
        ...
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    <div class="header">
                        <h2>Form Create Nilai</h2>
                    </div>
                    <div class="body">
                        <form id="form_validation" method="post" action="<?php echo base_url('index.php/nilai/add_nilai'); ?>">
                        <label>Periode</label>
                            <div class="form-group">
                                <select class="form-control show-tick" name="id_periode" required="">
                                   <?php foreach ($periode as $row) { ?>
                                   <option value="<?php echo $row->id_periode; ?>">PERIODE <?php echo $row->id_periode; ?> : <?php echo $row->periode_awal; ?> s/d <?php echo $row->periode_akhir; ?></option>
                                   <?php } ?>    
                               </select>
                           </div>
                            <label>Nilai Yang Diharapkan</label>
                            <div class="form-group">
                                <select class="form-control show-tick" name="level" required="">
                                   <?php foreach ($level as $row) { ?>
                                   <option value="<?php echo $row->id_maturity; ?>"><?php echo $row->level; ?></option>
                                   <?php } ?>    
                               </select>
                           </div>
                            ...
                            <div class="form-group form-float">
                                <div class="form-line">
                                    <input type="text" class="form-control" name="keterangan" required placeholder="Masukkan Keterangan memberikan nilai harapan">
                                </div>
                            </div>
